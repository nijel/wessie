...
while ($item = mysql_fetch_array ($id_result)){
    ...
    echo htmlspecialchars($name) . '</td><td>' . htmlspecialchars($desc) ;
    echo '</td><td>' . $depth;
...
if (isset($category)){
    echo '<em>' .$lang_name[$lng].':'.htmlspecialchars(get_category_name($category,$lng)).'</em>';
    echo '<table border="0"><tr><th>Title</th><th>Description</th><th>Depth</th><th>Rank</th><th>Expand</th><th colspan="4">Action</th></tr>'."\n";
    $even=0;
    add_childs(0,0);
    echo '</table><br/>';
    echo '<a href="menu_edit.php?category=',$category.'&amp;lng='.$lng.'">Add root item</a>';
}
